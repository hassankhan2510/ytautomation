...
4. **APIs & Services → OAuth consent screen** (newer console: **Google Auth Platform**).
   ...
   - **Data Access → Add scopes** → add **only** `https://www.googleapis.com/auth/youtube.upload` → Update → Save.
     - ⚠️ Do NOT add `youtube.readonly` (or other read scopes) — that triggers Google's mandatory
       app-verification wall. `youtube.upload` alone stays in the click-through "unverified" lane.
