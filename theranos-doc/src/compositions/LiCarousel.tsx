...
export const LiCarousel: React.FC = () => {
  ...
  const data = liData as unknown as { brand: string; handle?: string; name?: string; at?: string; avatar?: string; accent: string; bg?: [string, string, string]; angle?: number; motif?: string; cover?: string; shape?: string; slides: LiSlide[] };
  const slides = data.slides || [];
  const i = Math.min(frame, slides.length - 1);
  const slide = slides[i] || { type: "cover", title: data.brand };
  const meta = { brand: data.brand, handle: data.handle, name: data.name, at: data.at, avatar: data.avatar, accent: data.accent || "#4f8cff", bg: data.bg, angle: data.angle, motif: data.motif, cover: data.cover, shape: data.shape };
