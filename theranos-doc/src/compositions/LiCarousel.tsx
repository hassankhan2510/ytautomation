...
export const LiCarousel: React.FC = () => {
  ...
  const data = liData as unknown as { brand: string; handle?: string; accent: string; slides: LiSlide[] };
  const slides = data.slides || [];
  const i = Math.min(frame, slides.length - 1);
  const slide = slides[i] || { type: "cover", title: data.brand };
  const meta = { brand: data.brand, handle: data.handle, accent: data.accent || "#4f8cff" };
